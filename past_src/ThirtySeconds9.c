...
struct Me me    = {2, 6, A_GRID/2.0, 0}; // {j, i, radius, flag}
                                         //；i行j列に存在する半径rの円（自分）
struct Me enemy = {2, 6, A_GRID/2.0, 0}; // {j, i, radius, flag}
                                         //；i行j列に存在する半径rの円（敵）
...
void showStatusBar(int layerID/*, int seconds, int life<秒数とライフを受け取る必要>*/){ // StatueBar 部分を表示する関数
  ...
  //
  HgSetFont(HG_G, (int)STATUSBAR-20); // （フォント, 太さ）を予め指定.
  HgSetColor(HG_GREEN);
  HgText(WIDTH/20.0 - 2.0, 2.0, "TIME（00:30）", 30);
  HgSetColor(HG_WHITE);
  ...
  HgSetFont(HG_G, (int)STATUSBAR-20); // （フォント, 太さ）を予め指定.
  HgSetColor(HG_RED);
  HgText(WIDTH/20.0 + WIDTH/2.0 -2.0, 2.0, "LIFE（30 /30）", 30);
  HgSetColor(HG_WHITE);
  HgText(WIDTH/20.0 + WIDTH/2.0 -1.0, 1.0, "LIFE（30 /30）", 30);
  HgSetColor(HG_RED);
  HgText(WIDTH/20.0 + WIDTH/2.0     , 0.0, "LIFE（30 /30）", 30);
  // ------------------------------------------------------
...
void move(hgevent *ev, int layerID){
  ...
  { //CLOCKorSTOPを踏んだ時の処理. 関数化してもいいかも.
    ...
    HgImageSize(pic, &w, &h); //不要(?)
    HgWImagePut(0,
                A_GRID*(me.x-1) +(A_GRID/2.0), HEIGHT-(A_GRID*(me.y-1+1)) +(A_GRID/2.0),
                pic,
                (w/GRID)/A_GRID/4.5, /* scale(縮尺あわせ), なぜ4.5がちょうどいい？ */
                0);
  }
