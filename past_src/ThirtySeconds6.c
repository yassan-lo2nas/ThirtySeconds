...
// 現在のステージ情報をグローバル変数の２次変数で保持（リアルタイムに変動）.
int nowStage[GRID][GRID] = {{0}};
...
void showPac(double x, double y, double size, int direction, int openflag, int layerID)
{//自機,敵機を表示する関数
  switch(direction){
    case 0:
    ...
    case 2:
      ...
      break;
    case 3:
      if(openglag==0)HgWFanFill(layerID, x, y, size, 1.25*M_PI, 0.75*M_PI, 0);//左向き
