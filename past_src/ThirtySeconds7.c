...
struct Me me = {2, 0, A_GRID/2.0, 0}; // {j, i, radius, flag}
                                      //；i行j列に存在する半径rの円（自分）
struct Me enemy = {2, 0, A_GRID/2.0, 0}; // {j, i, radius, flag}
                                       //；i行j列に存在する半径rの円（敵）
...
void showStatusBar(int layerID/*, int seconds, int life<秒数とライフを受け取る必要>*/){ // StatueBar 部分を表示する関数
  ...
  //
  HgSetFont(HG_M, 30); // （フォント, 太さ）を予め指定.
  HgSetColor(HG_BLACK);
  HgText(WIDTH/4.0 +2,  2, "00:30", 30);
  HgSetColor(HG_GREEN);
  HgText(WIDTH/4.0   ,  0, "00:30", 30);
  // ------------------------------------------------------
  ...
  // HgSetFont(HG_M, 30); // （フォント, 太さ）を予め指定.
  HgSetColor(HG_BLACK);
  HgText(WIDTH/4.0 +2 + WIDTH/2.0 ,  2, "30 / 30", 30);
  HgSetColor(HG_RED);
  HgText(WIDTH/4.0    + WIDTH/2.0 ,  0, "30 / 30", 30);
  // ------------------------------------------------------
...
void showPac(double x, double y, double size, int direction, bool openflag, int layerID)
{//自機,敵機を表示する関数
  switch(direction){
    case 0:
...
int main(void) {
  ...
  do{
    ...
    showPac((double)(me.x-1) *A_GRID + (A_GRID/2.0),
            (double)(me.y)   *A_GRID + (A_GRID/2.0) + STATUSBAR,
            me.size,
            me.direction,
            openflag,
            layerID); //自機
