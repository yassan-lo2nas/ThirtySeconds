...
#define SCALE 4.5   // 読み込む画像の倍率, どういう計算のもとおこなっているのかがブラックボックス.
...
struct Me me    = {1, 1, A_GRID/2.0, 0, 30}; // {j, i, radius, flag}
                                         //；i行j列に存在する半径rの円（自分）
struct Me enemy = {1, 1, A_GRID/2.0, 0,  0}; // {j, i, radius, flag}
                                         //；i行j列に存在する半径rの円（敵）
...
void showStatusBar(int layerID/*, int passedSeconds, int life*/){ // StatueBar 部分を表示する関数
  layerID = 0; // 引数で受け取ったが, ベースレイヤに上書きするだけで良い.
  leftTime = MAX_SECONDS - passedSeconds;

  char buf[100];
  sprintf(&buf[0],"%02d",leftTime); // 数値を文字列に変換
  printf("残り時間は ... %s\n",&buf[0]); // 文字列を表示
  if (leftTime == 0){ending = GAMEOVER;}
  ...
  // 残り時間　文字列
  HgWText(layerID, WIDTH/20.0 - 2.0, 2.0, "TIME  00 :", 30);
  HgWSetColor(layerID, HG_WHITE);
  HgWText(layerID, WIDTH/20.0 - 2.0, 2.0, "TIME  00 :", 30);
  HgWSetColor(layerID, HG_GREEN);
  HgWText(layerID, WIDTH/20.0 - 2.0, 2.0, "TIME  00 :", 30);
  // 残り時間 時間
  HgWText(layerID, WIDTH/2.0 - WIDTH/8.0, 2.0, &buf[0], 30);
  HgWSetColor(layerID, HG_WHITE);
  HgWText(layerID, WIDTH/2.0 - WIDTH/8.0, 2.0, &buf[0], 30);
  HgWSetColor(layerID, HG_GREEN);
  HgWText(layerID, WIDTH/2.0 - WIDTH/8.0, 2.0, &buf[0], 30);
  ...
  for(int i=0; i<GRID; i++){
    for(int j=0; j<GRID; j++){
      ...
      HgWImagePut(layerID,
                  ...
                  pic,
                  (w/GRID)/A_GRID/SCALE, /* scale(縮尺あわせ), なぜ4.5がちょうどいい？ */
                  0);
    }
  }
...
void OperationByKeyDown(hgevent *ev, int layerID){
  ...
  switch (ev->ch) {//キー操作が以下の場合
    case HG_U_ARROW:
      ...
      if(y-1 < 0){printf("画面外には進めません。\n");break;} // マップ外は移動出来ないように除く.
  ...
  { //CLOCKorSTOPを踏んだ時の処理. 関数化してもいいかも.
    if(nowStage[me.y-1][me.x-1] == CLOCK){
      printf("時計ゲット!１０秒追加\n");
      /*　ここにタイマーを10秒増やす処理　 */
      if(passedSeconds >= 10) {passedSeconds -= 10;}
      else{passedSeconds = 0;}
    }
...
int main(void) {
  ...
  bool openflag = true; // bool={true: open, false:close} のフラグ.
                        // パックマンの口開閉のフラグ.
  double passedTime = 0.0; // 経過時間を保存する変数.
          ...
          if(countTimer % TIMES_OF_SECOND == 0){//(counter*INTERVAL)=1.0[s]なら
            ...
            for(int i=0; i<GRID; i++){ // 偶数秒ごとに床を切り替え, 関数化してもいいかも.
              for(int j=0; j<GRID; j++){
                ...
                printf("%2d, ", nowStage[i][j]); // 確認用 消しても良い
              }
              printf("\n"); // 確認用 消しても良い
            }
            /* 関数化してもいいかも（終了） */
